...
//admin
        ...
        public bool InsertProduct(Product product)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "INSERT INTO Products (Title, Description, Price, Quantity, ImageName, CategoryId, OfferId) VALUES (@title, @description, @price, @quantity, @imageName, @categoryId, @offerId);";
        ...
        public bool InsertCategory(ProductCategory category)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                command.CommandText = query;
                command.Parameters.Add("@category", SqlDbType.NVarChar).Value = category.Category;
                command.Parameters.Add("@subCategory", SqlDbType.NVarChar).Value = category.SubCategory;
...
//user
            ...
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (quantity <= 0)
                        {
                            // If quantity is 0 or below, remove the cart item
                            string removeQuery = "DELETE FROM CartItems WHERE CartId IN (SELECT CartId FROM Carts WHERE UserId = @userId AND Ordered = 'false') AND ProductId = @productId;";
                            ...
                            // Commit the transaction if the removal was successful
                            if (rowsAffected > 0)
                                transaction.Commit();
                        ...
                        else
                        {
                            ...
                            string updateQuery = "UPDATE CartItems SET Quantity = @quantity WHERE CartId IN (SELECT CartId FROM Carts WHERE UserId = @userId AND Ordered = 'false') AND ProductId = @productId;";
        ...
        public bool RemoveCartItem(int userId, int productId)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                connection.Open();
                string query = "DELETE FROM CartItems WHERE CartId IN (SELECT CartId FROM Carts WHERE UserId=@uid AND Ordered='false') AND ProductId=@pid;";
                command.CommandText = query;
        ...
        public bool IncrementCartItemQuantity(int userId, int productId)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                SqlCommand command = new SqlCommand
                ...
                string query = "UPDATE CartItems SET Quantity = Quantity + 1 WHERE CartId = (SELECT CartId FROM Carts WHERE UserId = @userId AND Ordered = 'false') AND ProductId = @productId;";
                command.CommandText = query;
        ...
        public bool DecrementCartItemQuantity(int userId, int productId,int quantity)
        {
            if (quantity < 2)
            {
                return false;
            ...
            {          
                using (SqlConnection connection = new SqlConnection(dbconnection))
                {
                    ...
                    if(quantity==0)
                    {
                        string query1 = "DELETE FROM CartItems WHERE CartId IN (SELECT CartId FROM Carts WHERE UserId=@uid AND Ordered='false') AND ProductId=@pid;";
